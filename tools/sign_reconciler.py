...
class SignReconciler:
    ...
    def load_data(self) -> bool:
        ...
        try:
            ...
            self.log(f"Loaded sign_database.json: {self.sigla_data['statistics']['total_signs']} signs")
        except Exception as e:
    ...
    def build_mapping(self) -> dict:
        ...
        # Generate statistics
        stats = {
            'syllabograms_mapped': len([k for k, v in self.phonetic_to_ab.items()
                                        if v.get('type') == 'syllabogram']),
        ...
        if self.signs_data:
            for sign_key, sign_data in self.signs_data.get('signs', {}).items():
                ...
                if sign_upper in self.unified_signs:
                    self.unified_signs[sign_upper]['frequency_corpus'] = sign_data.get('total_occurrences', 0)
                    self.unified_signs[sign_upper]['position_frequency'] = sign_data.get('position_frequency', {})
    ...
    def save_mapping(self, output_path: Path = None):
        ...
        output = {
            ...
            'statistics': {
                'syllabograms': len([k for k, v in self.phonetic_to_ab.items()
                                     if v.get('type') == 'syllabogram']),
    ...
    def generate_report(self) -> str:
        ...
        lines.append("-" * 40)
        lines.append(f"Syllabograms with AB mapping: {len([k for k, v in self.phonetic_to_ab.items() if v.get('type') == 'syllabogram'])}")
        ...
        if self.unmapped_signs:
            ...
            for sign_info in sorted(self.unmapped_signs, key=lambda x: -x.get('occurrences', 0)):
                lines.append(f"{sign_info['sign']:<15} {sign_info.get('occurrences', 0):<12} {sign_info.get('note', '')[:40]}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Reconcile Linear A sign classification systems"
    )
    parser.add_argument(
        '--build', '-b',
        action='store_true',
    ...
    parser.add_argument(
        '--lookup', '-l',
        type=str,
        help='Look up sign by phonetic value (e.g., KU, RA, OLE)'
    )
    parser.add_argument(
        '--ab',
        type=str,
        help='Look up sign by AB number (e.g., AB81, AB02)'
    )
    parser.add_argument(
        '--word', '-w',
        type=str,
        help='Normalize a word showing AB mappings (e.g., KU-RO)'
    )
    parser.add_argument(
        '--validate',
        action='store_true',
        help='Validate all corpus signs against sign database'
    )
    parser.add_argument(
        '--report', '-r',
        action='store_true',
        help='Generate human-readable reconciliation report'
    )
    parser.add_argument(
        '--output', '-o',
        type=str,
