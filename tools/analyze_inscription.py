...
class InscriptionAnalyzer:
    ...
    def generate_first_principles_verification(self) -> dict:
        ...
        verification = {
            "p1_kober": {
                "status": "PASS",
                "evidence": "Structural analysis preceded language assumptions",
            },
        ...
        if anchors:
            verification["p3_anchors"]["evidence"] = (
                f"Identified {len(anchors)} anchors at levels {set(a['level'] for a in anchors)}"
            )
        else:
    ...
    def generate_markdown_report(self) -> str:
        ...
        lines.append(
            f"- **Has Total Line (KU-RO)**: {'Yes' if structure['has_total_line'] else 'No'}"
        )
        ...
        syllabic_words = [
            w for w in self.analysis["words"] if not w["is_logogram"] and w["syllable_count"] > 1
        ]
        if syllabic_words:
            lines.append("\n### Key Syllabic Words")
            ...
            for word in syllabic_words[:15]:
                cognates = (
                    ", ".join([c.get("root", c.get("type", "")) for c in word["cognates"]]) or "-"
                )
        ...
        lines.append(
            "- For full multi-hypothesis testing, run `hypothesis_tester.py` on specific words."
        )
        lines.append(
            "- For corpus verification, run `corpus_lookup.py --verify` on specific words."
        )
...
def main():
    parser = argparse.ArgumentParser(description="Automated Linear A inscription analysis")
    parser.add_argument("inscription", help="Inscription ID to analyze (e.g., HT13, ZA4)")
