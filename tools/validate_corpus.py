...
class CorpusValidator:
    ...
    def __init__(self, verbose=False, quiet=False):
        ...
        self.known_chronologies = {
            'LMIB', 'LMIA', 'LMI', 'MMII', 'MMIII', 'MMIIIB', 'MMIIIA', 'MMIA',
            'LMIIIA', 'LBI', 'Geometric', 'LMII', 'MMIB'
        }
    ...
    def validate_duplicates(self):
        ...
        if duplicates:
            for content, ids in list(duplicates.items())[:5]:  # Show first 5
                self.result.add_warning(
                    f"Potential duplicate content: {', '.join(ids[:3])}",
                    f"Hash: {content[:50]}..."
                )
        ...
        for sign, data in self.signs['signs'].items():
            ...
            if pos_sum != total:
                inconsistencies.append(
                    f"{sign}: total={total} but positions sum to {pos_sum}"
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Validate Linear A corpus data integrity"
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
    ...
    parser.add_argument(
        '--report-only',
        action='store_true',
        help='Only generate report, suppress console output'
    )
    ...
    validator = CorpusValidator(
        ...
        quiet=args.quiet or args.report_only
    )
