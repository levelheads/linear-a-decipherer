...
class SyntaxAnalyzer:
    ...
    def classify_text_type(self, insc_id: str, words: List[str]) -> str:
        ...
        admin_score = sum(1 for marker in TEXT_PATTERNS['administrative']['markers']
                         if any(marker in w for w in words_upper))
        if admin_score >= 2:
            return 'administrative'

        # Check for inventory markers
        inv_score = sum(1 for marker in TEXT_PATTERNS['inventory']['markers']
                       if any(marker in w for w in words_upper))
        if inv_score >= 2:
            return 'inventory'
    ...
    def extract_slot_grammar(self, text_type: str) -> dict:
        ...
        inscriptions = self.results['text_classification'].get('inscriptions', {}).get(text_type, [])
        ...
        for slot, counter in slot_patterns.items():
            grammar['slots'][slot] = [
                {'word': w, 'count': c, 'percentage': round(c / total_analyzed * 100, 1) if total_analyzed > 0 else 0}
    ...
    def analyze_particle_positions(self) -> dict:
        ...
        for particle, data in particle_data.items():
            ...
            analysis[particle] = {
                ...
                'positions': {
                    'initial': {'count': data['initial'], 'pct': round(data['initial'] / total * 100, 1)},
                    'medial': {'count': data['medial'], 'pct': round(data['medial'] / total * 100, 1)},
    ...
    def score_word_order_hypotheses(self) -> dict:
        ...
        if final_count >= 3:
            scores['SOV']['score'] += 2
            scores['SOV']['evidence'].append(f'{final_count} particles are word-final (suffix-heavy = SOV tendency)')
        ...
        analysis = {
            ...
            'confidence': 'PROBABLE' if ranking[0][1]['final_score'] > ranking[1][1]['final_score'] + 1 else 'POSSIBLE',
            'interpretation': '',
        }
        ...
        best = ranking[0][0]
        if best == 'VSO':
            analysis['interpretation'] = 'Linear A may follow Verb-Subject-Object order (Semitic influence)'
        elif best == 'SOV':
            analysis['interpretation'] = 'Linear A may follow Subject-Object-Verb order (Luwian/Anatolian pattern)'
        else:
            analysis['interpretation'] = 'Linear A may follow Subject-Verb-Object order (isolate pattern)'
    ...
    def print_summary(self):
        ...
        for particle, data in particles.items():
            print(f"  {particle}: {data['dominant_position']} dominant ({data['total_occurrences']} occ)")
...
def main():
    ...
    parser.add_argument('--administrative', action='store_true', help='Analyze administrative texts')
