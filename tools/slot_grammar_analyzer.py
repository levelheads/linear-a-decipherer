    ...
    def extract_triplets(self) -> List[dict]:
        ...
        for insc_id, data in self.corpus.get('inscriptions', {}).items():
            ...
            for i in range(len(valid_tokens) - 2):
                ...
                if self.is_syllabic(t1['word']) and \
                   self.is_logogram(t2['word']) and \
                   self.is_numeral(t3['word']):
            ...
            for i in range(len(valid_tokens) - 2):
                ...
                if self.is_logogram(t1['word']) and \
                   self.is_numeral(t2['word']) and \
                   self.is_syllabic(t3['word']):
...
class ConsistencyValidator:
    ...
    def validate_suffix_consistency(self, triplets: List[dict], suffix: str) -> dict:
        ...
        positions = Counter(
            'EARLY' if t['position_in_inscription'] < t['total_tokens'] // 3
            else 'MIDDLE' if t['position_in_inscription'] < 2 * t['total_tokens'] // 3
            else 'LATE'
            for t in matching
        )
    ...
    def validate_role_assignment(self, triplets: List[dict],
                                  match_results: dict,
        ...
        consistent_roles = sum(1 for r in validation['by_role'].values()
                               if r.get('verdict') == 'CONSISTENT')
...
class SynthesisEngine:
    ...
    def rank_hypotheses(self, match_results: dict) -> List[dict]:
        ...
        for hyp in hypotheses:
            ...
            total_avg = sum(s.get('average', 0) for s in scores.values()) / len(scores) if scores else 0
            best_role = max(scores.items(), key=lambda x: x[1].get('average', 0)) if scores else (None, {})
    ...
    def identify_discriminating_patterns(self, match_results: dict,
                                         triplets: List[dict]) -> List[dict]:
        """
        ...
        suffix_analysis = defaultdict(lambda: {h: 0 for h in ['luwian', 'semitic', 'pregreek', 'protogreek']})
    ...
    def determine_best_interpretation(self, match_results: dict,
                                       validation: dict,
                                       hypothesis_ranking: List[dict]) -> dict:
        """
        ...
        if hypothesis_ranking:
            ...
            if best_hyp['best_role_score'] > 0.5:
                interpretation['role_confidence'] = 'HIGH' if best_hyp['best_role_score'] > 0.7 else 'MEDIUM'
            else:
...
class SlotGrammarAnalyzer:
    ...
    def run_validation(self) -> dict:
        ...
        top_suffixes = list(self.results['slots_extracted'].get('slot_suffix_frequencies', {}).keys())[:10]
        suffix_validation = {}
        ...
        p6_status = role_validation.get('first_principle_check', {}).get('P6_corpus_consistency', 'UNKNOWN')
    ...
    def run_synthesis(self) -> dict:
        ...
        for i, hyp in enumerate(self.results['hypothesis_ranking'], 1):
            print(f"  {i}. {hyp['hypothesis'].upper()}: {hyp['overall_score']:.4f} (best role: {hyp['best_role']})")
        ...
        for dp in self.results['discriminating_patterns'][:5]:
            print(f"  -{dp['suffix']}: favors {dp['favors_hypothesis'].upper()} (disc={dp['discrimination']:.3f})")
        ...
        # First Principles verification
        self.results['first_principles_verification'] = self.results['best_interpretation']['first_principles_verification']
    ...
    def save_results(self, output_path: Path):
        ...
        if 'slots_extracted' in output and 'triplets' in output['slots_extracted']:
            output['slots_extracted']['triplets_sample'] = output['slots_extracted']['triplets'][:50]
        ...
        if 'match_results' in output and 'word_matches' in output['match_results']:
            ...
            output['match_results']['word_matches_count'] = len(self.results['match_results']['word_matches'])
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze Linear A slot grammar patterns"
    )
    parser.add_argument(
        '--extract',
        action='store_true',
        help='Extract commodity slots only'
    )
    parser.add_argument(
        '--predict',
        action='store_true',
        help='Generate prediction matrix only'
    )
    parser.add_argument(
        '--match',
        action='store_true',
        help='Run pattern matching only'
    )
    parser.add_argument(
        '--validate',
        action='store_true',
        help='Run consistency validation only'
    )
    parser.add_argument(
        '--all', '-a',
        action='store_true',
