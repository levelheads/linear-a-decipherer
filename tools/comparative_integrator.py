...
class ComparativeIntegrator:
    ...
    def validate_reading(
        self, linear_a_word: str, proposed_reading: str, hypothesis: str = "any"
    ) -> ValidationResult:
        """
        ...
        if supporting_matches:
            avg_relevance = sum(m.relevance_score for m in supporting_matches) / len(
                supporting_matches
            )
...
def main():
    ...
    parser.add_argument("--query", "-q", type=str, help="Query comparative corpora for a term")
    parser.add_argument(
        "--validate", type=str, help='Validate a reading (format: "WORD = meaning")'
    )
    parser.add_argument(
        "--hypothesis",
        type=str,
    ...
    parser.add_argument(
        "--update-cache", action="store_true", help="Save current data to cache files"
    )
