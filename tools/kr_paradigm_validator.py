...
class KRParadigmValidator:
    ...
    def find_all_kr_forms(self) -> List[dict]:
        ...
        for insc_id, data in self.corpus.get("inscriptions", {}).items():
            ...
            valid_words = [
                (i, w) for i, w in enumerate(words) if w and w not in ["\n", "𐄁", "", "—", "≈", "𐝫"]
            ]
            ...
            for pos, (orig_idx, word) in enumerate(valid_words):
                ...
                if pattern:
                    ...
                    prev_type = (
                        self._classify_word(context_before[-1]) if context_before else "NONE"
                    )
    ...
    def map_kr_contexts(self) -> dict:
        ...
        context_analysis["common_sequences"] = [
            {"sequence": seq, "count": count} for seq, count in sequence_counts.most_common(20)
        ]
    ...
    def print_summary(self):
        ...
        for pattern, count in sorted(
            self.results["form_counts"].items(), key=lambda x: x[1], reverse=True
        ):
...
def main():
    parser = argparse.ArgumentParser(description="Validate K-R paradigm across Linear A corpus")
    parser.add_argument("--all", "-a", action="store_true", help="Run complete validation")
    parser.add_argument(
        "--output",
        "-o",
        type=str,
