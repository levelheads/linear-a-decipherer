...
| Item | Reason |
|------|--------|
| `data/*.json` (generated) | Regenerated by tools (gitignored via `data/*`) |
...
**Exception**: `data/anchors.json` and `data/reading_dependencies.json` ARE committed — they are curated reference data, not generated output. See `.gitignore` negation rules.
...
```
data/*                  # Generated JSON files (except curated reference data below)
__pycache__/            # Python bytecode
*.log                   # Log files
.DS_Store               # macOS metadata
```

**Committed exceptions in `data/`**: `anchors.json`, `reading_dependencies.json` (curated reference data required by promotion board and dependency tracking).
...
[ ] 6. CI is green on main
    - gh run list --limit 1 (must show 'success')
    - If new tools added: verify CI generates all required data files

[ ] 7. Tag AFTER committing release changes
    - Commit first, tag second, push both

[ ] 8. Verify tag CI run passes
    - gh run watch <run-id> --exit-status
    - If tag CI fails: fix, retag with git tag -f, force-push tag
